<?php 
	if(isset($_GET['mahdduyet'])){
		...
		$update=mysqli_query($conn, "UPDATE dathang SET TrangThai='Đã xử lí' WHERE SoDonDH='$mahdduyet' ");
		if($update){
			echo "<script>
	...
	if(isset($_GET['mahdxoa'])){
		...
		$xoa=mysqli_query($conn,"DELETE FROM dathang WHERE SoDonDH = '$mahdxoa'");
		if($xoa){
			echo "<script>
...
<div class="tatca_HangHoa">
	...
	<?php $sql=mysqli_query($conn,"SELECT * FROM dathang WHERE TrangThai='Chưa xử lí'");
	       $row=mysqli_num_rows($sql);
	       $sql1=mysqli_query($conn,"SELECT * FROM dathang WHERE TrangThai='Đã xử lí'");
	...
	<table border="1" style="text-align: center;margin-top: 40px" width="100%" cellpadding ="35px">
		...
		$sql=mysqli_query($conn,"SELECT * FROM dathang  order by MSKH limit ".$item_per_page." offset ".$offset."");
		$total_product=mysqli_query($conn, "SELECT * FROM dathang ");
		...
		if(mysqli_num_rows($sql)>0){
			...
			?>
				<tr>
					...
					<td><?=$row['NgayDH']?></td>
					<td><?=$row['TrangThai']?></td>
					<td><a href="admin.php?content1=chitiet_hoadon&mahdchitiet=<?=$row['MSKH']?>">Chi tiết hóa đơn</a></td>

					
					<td><a href="admin.php?content1=qlhd&mahdduyet=<?=$row['MSKH']?>">Duyệt</a></td>
					<td><a href="admin.php?content1=qlhd&mahdxoa=<?=$row['MSKH']?>">Xóa</a></td>
