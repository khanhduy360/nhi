<?php
	if(isset($_GET['idxoand'])){
		...
		$xoand=mysqli_query($conn,"DELETE FROM khachhang WHERE MaND='$idxoand'");
		if($xoand){
			echo "<script>
...
<div class="tatca_HangHoa">
	...
	<?php $result=mysqli_query($conn,"SELECT * FROM khachhang");
	...
	<table border="1" style="text-align: center;margin-top: 40px" width="100%" cellpadding ="35px">
				<tr>
					<th id="MaND">Mã người dùng</th>
					<th>Tên người dùng</th>
					<th>User name</th>
					<th>SDT</th>
					<th>Địa chỉ</th>
					<th id="email">Email</th>
					<th id="gioitinh">Giới tính</th>
					<th id="ngaydangki">Ngày đăng kí</th>
		...
		$sql=mysqli_query($conn,"SELECT * FROM khachhang WHERE phanquyen='1' order by MaND limit ".$item_per_page." offset ".$offset."");
		$total_product=mysqli_query($conn, "SELECT * FROM khachhang WHERE phanquyen='1'");
