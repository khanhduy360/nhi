...
<body>
	<?php
		if(isset($_GET['idxoa'])){
			$MSHH=$_GET['idxoa'];
			$xoa=mysqli_query($conn, "DELETE FROM HangHoa WHERE MSHH = '$MSHH' ");
			echo "<script>
	...
	<?php 
		...
		$sql=mysqli_query($conn,"SELECT * FROM HangHoa order by MaNhom limit ".$item_per_page." offset ".$offset."");
		$total_product=mysqli_query($conn, "SELECT * FROM HangHoa");
		...
		if(mysqli_num_rows($sql)>0){
			...
			?>
				<tr>
					...
					<td><?=$row['loai']?></td>
					<td><?=$row['TenHH']?></td>
					<td><img src="../img/<?=$row['Hinh']?>" style="width: 50px;height: 50px;"></td>
					...
					<td><a href="admin.php?content1=update_sp&idupdate=<?=$row['MSHH']?>"><img src="../img/edit.png" style="width: 50px;height: 50px"></a></td>
					<td><a href="admin.php?content1=qlsp&idxoa=<?=$row['MSHH']?>"><img src="../img/delete.png" style="width: 50px;height: 50px"></a></td>
				</tr>
...
<table  style="margin-bottom:30px">
     <?php 
       $result1=mysqli_query($conn,"SELECT sum(tongso) FROM HangHoa"); 
       $row1=mysqli_fetch_assoc($result1);
       $result2=mysqli_query($conn,"SELECT sum(daban) FROM HangHoa"); 
       $row2=mysqli_fetch_assoc($result2); 
       $result3=mysqli_query($conn,"SELECT sum(conlai) FROM HangHoa"); 
    ...
    <tr style="border-bottom:2px solid #dfe6e9">
        <?php  $tongloinhuan = 0;
        $result5=mysqli_query($conn,"SELECT * FROM HangHoa");
